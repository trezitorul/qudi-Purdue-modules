...
class Scan1DWidget(_BaseScanWidget):
    ...
    def __init__(self,
        ...
        self.plot_widget_0 = RubberbandZoomSelectionPlotWidget(allow_tracking_outside_data=True)
        self.plot_widget_0.addItem(self.plot_item)
        self.plot_widget_0.set_selection_mutable(True)
        print("Here 107")
        self.plot_widget_0.add_marker_selection(position=(0, 0),
                                              mode=self.plot_widget_0.SelectionMode.X)
        self.plot_widget_0.add_marker_selection(position=(100, 100))
        self.plot_widget_0.sigMarkerSelectionChanged.connect(self._markers_changed)
        self.channel_selection_combobox.currentIndexChanged.connect(self._data_channel_changed)
        self.plot_widget_0.sigZoomAreaApplied.connect(self._zoom_applied)
        self.plot_widget_0.setLabel('bottom', text=axes[0].name.title(), units=axes[0].unit)
        self.plot_widget_0.setLabel('left', text=channels[0].name, units=channels[0].unit)


        # self.plot_widget_1 = RubberbandZoomSelectionPlotWidget(allow_tracking_outside_data=True)
        # self.plot_widget_1.addItem(self.plot_item)
        # self.plot_widget_1.set_selection_mutable(True)
        # self.plot_widget_1.add_marker_selection(position=(0, 0),
        #                                       mode=self.plot_widget_1.SelectionMode.X)
        # self.plot_widget_1.sigMarkerSelectionChanged.connect(self._markers_changed)
        # self.channel_selection_combobox.currentIndexChanged.connect(self._data_channel_changed)
        # self.plot_widget_1.sigZoomAreaApplied.connect(self._zoom_applied)
        # self.plot_widget_1.setLabel('bottom', text=axes[0].name.title(), units=axes[0].unit)
        # self.plot_widget_1.setLabel('left', text=channels[0].name, units=channels[0].unit)

        self.layout().addWidget(self.plot_widget_0, 1, 0, 1, 4)
        # self.layout().addWidget(self.plot_widget_1, 2, 0, 1, 4)

        # disable buggy pyqtgraph 'Export..' context menu
        self.plot_widget_0.getPlotItem().vb.scene().contextMenu[0].setVisible(False)

    @property
    def marker_position(self) -> float:
        return self.plot_widget_0.marker_selection[self.plot_widget_0.SelectionMode.X][0]

    def set_marker_position(self, position: float) -> None:
        self.plot_widget_0.move_marker_selection((position, 0), 0)

    def toggle_zoom(self, enable: bool) -> None:
        if enable:
            mode = self.plot_widget_0.SelectionMode.X
        else:
            mode = self.plot_widget_0.SelectionMode.Disabled
        self.plot_widget_0.set_rubberband_zoom_selection_mode(mode)

    def toggle_marker(self, show: bool) -> None:
        if show:
            self.plot_widget_0.show_marker_selections()
        else:
            self.plot_widget_0.hide_marker_selections()

    @property
    def marker_bounds(self) -> Union[None, List[Union[None, Tuple[float, float]]]]:
        return self.plot_widget_0.selection_bounds

    def set_marker_bounds(self,
                          bounds: Union[None, List[Union[None, Tuple[float, float]]]]
                          ) -> None:
        self.plot_widget_0.set_selection_bounds(bounds)

    def set_plot_range(self,
                       x_range: Optional[Tuple[float, float]] = None,
                       y_range: Optional[Tuple[float, float]] = None
                       ) -> None:
        self.plot_widget_0.setRange(xRange=x_range, yRange=y_range)
    ...
    def _markers_changed(self, markers) -> None:
        position = markers[self.plot_widget_0.SelectionMode.X][0]
        self.sigMarkerPositionChanged.emit(position)

    @QtCore.Slot(QtCore.QRectF)
    def _zoom_applied(self, zoom_area) -> None:
        if self.plot_widget_0.rubberband_zoom_selection_mode == self.plot_widget_0.SelectionMode.X:
    ...
    def _data_channel_changed(self) -> None:
        if self._scan_data is not None:
            ...
            self.plot_widget_0.setLabel('left',
