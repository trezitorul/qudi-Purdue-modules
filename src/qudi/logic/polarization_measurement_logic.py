...
class polarization_measurement_logic(LogicBase):
    ...
    counter = Connector(interface='counter_logic', missing='error')
    ...
    def start_measurement_loop(self):
        ...
        for angle in self.scan_angles:        
            ...
            self.pol_motor.set_position(angle)
            print("Angle Set")
            counts=self.counter.get_counts(self.int_time, self.counter_channels)
