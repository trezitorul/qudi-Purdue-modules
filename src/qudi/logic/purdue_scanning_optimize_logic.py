...
class ScanningOptimizeLogic(LogicBase):
    ...
    def on_activate(self):
        ...
        self.log.info(f"Opt settings at startup, type {type(self._scan_range)} {self._scan_range, self._scan_resolution}")
        self._avail_axes = tuple(axes.values())
        self.log.info(f"Opt available Axes {self._avail_axes}")
    ...
    def _scan_state_changed(self, is_running, data, caller_id):
        ...
        with self._thread_lock:
            if is_running or self.module_state() == 'idle' or caller_id != self.module_uuid:
                ...
                self.log.info(f"Trying to fit on data after scan of dim {data.scan_dimension}")
                try:
                    ...
                    self.log.info(f"Optimizer issuing position update: {position_update}")
    ...
    def _get_pos_from_2d_gauss_fit(self, xy, data):
        ...
        try:
            ...
            self.log.warning('2D Gaussian Fit Failed')
            return (x_middle, y_middle), None, None
        
        self.log.info("2D Optimization Successful: " + str((fit_result.best_values['center_x'],
                fit_result.best_values['center_y'])))
    ...
    def _get_pos_from_1d_gauss_fit(self, x, data):
        ...
        try:
            ...
            self.log.warning('1D Gaussian Fit Failed')
            return (middle,), None, None
        self.log.info("1D Optimization Successful: " + str(fit_result.best_values['center']))
